use wavetable::{WavHandler, WtManager, WtReader};

use flexi_logger::{Logger, opt_format};

/// Convert a wave file to a different format.
fn main () {
    // Start as "RUST_LOG=info cargo run --example bandlimit <filename>"
    // to show log info
    ...
    let result = WtReader::create_wavetable(&wav_file, Some(2048));
    match result {
        Ok(wt_ref) => {
            println!("Got wavetable with {} tables, for {} octaves, with {} samples and {} values per octave",
                ...
                wt_ref.num_samples,
                wt_ref.num_values);
            let wt_ref = WtManager::bandlimit(wt_ref, 11, 44100.0);
